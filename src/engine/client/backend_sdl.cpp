...
int CGraphicsBackend_SDL_GL::Init(const char *pName, int *pScreen, int *pWidth, int *pHeight, int *pRefreshRate, int *pFsaaSamples, int Flags, int *pDesktopWidth, int *pDesktopHeight, int *pCurrentWidth, int *pCurrentHeight, IStorage *pStorage)
{
	...
	*pFsaaSamples = 0;
	...
	// TClient: Removed automatic fallback from Vulkan to OpenGL
	// The old code would prevent Vulkan from being selected if any backend was previously initialized
	// Now we only fallback if Vulkan initialization actually fails later in the process

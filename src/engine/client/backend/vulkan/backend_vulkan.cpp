...
protected:
	...
	const char *CheckVulkanCriticalError(VkResult CallResult)
	{
		...
		case VK_SUBOPTIMAL_KHR:
			// TClient: Don't recreate swap chain on VK_SUBOPTIMAL_KHR to prevent screen flashing on Alt+Tab
			// The swap chain is still functional, just not optimal. Only recreate on VK_ERROR_OUT_OF_DATE_KHR.
			if(IsVerbose())
			{
				log_debug("gfx/vulkan", "Swap chain is suboptimal but still usable, not recreating.");
			}
			break;
		default:

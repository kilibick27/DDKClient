...
#if defined(CONF_FAMILY_WINDOWS) && !defined(BC_DISABLE_WASAPI)
	...
	void WorkerMain()
	{
		...
		auto EnsureCapture = [&]() -> bool {
			if(pCaptureClient && pAudioClient)
				return true;
			Cleanup();
			if(FAILED(CoCreateInstance(__uuidof(MMDeviceEnumerator), nullptr, CLSCTX_ALL, __uuidof(IMMDeviceEnumerator), reinterpret_cast<void **>(&pEnumerator))))
			{
				++ConsecutiveFailures;
			...
			if(FAILED(pDevice->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr, reinterpret_cast<void **>(&pAudioClient))))
			{
				++ConsecutiveFailures;
				StoreBackendState(ConsecutiveFailures >= 6 ? EVisualizerBackendStatus::UNAVAILABLE : EVisualizerBackendStatus::CONNECTING);
				return false;
			...
			if(FAILED(pAudioClient->GetService(__uuidof(IAudioCaptureClient), reinterpret_cast<void **>(&pCaptureClient))))
			{
				++ConsecutiveFailures;
				StoreBackendState(ConsecutiveFailures >= 6 ? EVisualizerBackendStatus::UNAVAILABLE : EVisualizerBackendStatus::CONNECTING);
				return false;
